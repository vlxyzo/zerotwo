...
export class DatabaseManager {
	private static readonly defaultDirectory = join(process.cwd(), 'database');
	...
	public async getUser(telegramId: Parameters<typeof toDbId>[0]): Promise<UserRecord | undefined> {
		...
		try {
			...
			log.warning(`Supabase read failed; using local users mirror: ${errorMessage(error)}`);
	...
	public async upsertUser<TInfo extends Json>(
		userData: UserInsert<TInfo>
	): Promise<void> {
		...
		try {
			...
			log.warning(`Supabase upsert failed; queueing local update: ${errorMessage(error)}`);
	...
	private async readUsers(): Promise<UserRecord[]> {
		return this.readJson(this.usersFile, value => {
	...
	private async readQueue(): Promise<UserPayload[]> {
		return this.readJson(this.queueFile, value => {
	...
	private normalizePayload<TInfo extends Json>(
		payload: UserInsert<TInfo>
	): UserPayload {
		return {
			...payload,
	...
	private startSweeper(): void {
		this.sweepTimer = setInterval(() => {
			...
			void this.syncOfflineQueue().catch(error => {
				log.warning(`Offline queue sweep failed: ${errorMessage(error)}`);
			});
